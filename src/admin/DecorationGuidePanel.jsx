...
export default function DecorationGuidePanel({ elementId }) {
  ...
  async function build(force) {
    ...
    try {
      ...
      if (res?.notModelled) {
        ...
        /* ⚠️ The steps and the picture are two paid calls and the second can fail on its own. It
           used to fail SILENTLY — `stages_key` null, `{ ok: true }` back, and the panel just drew
           no image, so a step that cost money looked exactly like one nobody asked for. Reported as
           "it's only generating text".
           The reason rides on the guide row (`stages_error`), so it is the same answer here and on
           a reload tomorrow — no second copy in component state that a refresh would silently lose. */
        setMsg(res.guide?.stages_error
          ? { ok: false, text: 'Steps generated and saved — the picture failed.' }
  ...
  /* Why there is no picture, verbatim from the provider, stored on the row by the last build.
     ⚠️ ADMIN-ONLY — it names models and quotes internal API text. The API keeps it out of both
     baker-reachable reads (CRAFT_FIELDS vs ADMIN_CRAFT_FIELDS, plus `forBaker` on the built row),
     so X-Ray never sees this string; a baker with no picture simply gets the steps. */
  const stagesError = data?.guide?.stages_error ?? null;
      ...
      {guide && (
        <div style={{ marginBottom: 12 }}>
          ...
          {stagesUrl ? (
            ...
            /* A guide WITHOUT a picture, said out loud. Both halves come from the row: `stages_key`
               null is THAT there is none, `stages_error` is WHY — so a reload says the same thing,
               and a guide generated before the column existed shows the first half alone. */
            <div style={{ marginBottom: 14, padding: '10px 12px', borderRadius: 8, background: '#FEF3C7', color: '#92400E', fontSize: 12, lineHeight: 1.5 }}>
              <b>Steps only — this guide has no picture.</b>{' '}
              The sheet is generated by a second call that can fail on its own; the steps above are
              unaffected and are saved.
              {stagesError && (
                <div style={{ marginTop: 6, fontFamily: 'ui-monospace, Menlo, monospace', fontSize: 11, wordBreak: 'break-word' }}>
                  {stagesError}
                </div>
              )}
              <div style={{ marginTop: 6 }}>
                Rebuild to try again. If it fails the same way twice the artwork itself is being
                refused, and no number of retries will change that.
              </div>
            </div>
          )}
