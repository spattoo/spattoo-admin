...
export default function DecorationGuidePanel({ elementId }) {
  ...
  // Why the last build produced no picture, straight from the provider. Lives only as long as the
  // panel: it describes ONE attempt, and a reload is a different question (answered below from the
  // stored row, which knows there is no picture but not why).
    ...
    try {
      ...
      if (res?.notModelled) {
        ...
        /* ⚠️ The steps and the picture are two paid calls and the second can fail on its own. It
           ...
           "it's only generating text". The provider's own words, unedited: they are what decides
           whether Rebuild is worth pressing (a timeout will pass, a refusal never will). */
        setImageError(res.imageError ?? null);
        setMsg(res.imageError
      ...
      {guide && (
        <div style={{ marginBottom: 12 }}>
          ...
          {stagesUrl ? (
            ...
            /* A guide WITHOUT a picture, said out loud. The row knows this on its own (`stages_key`
               is null), so it survives a reload — `imageError` adds the reason when the failure
               happened in this session, and is simply absent for a guide generated days ago. */
